...
$db = mysqli_connect("localhost","root","","whatwe_sell");
...
$select_customer_history = "select * from customers_history where ip_address='$ip_address' and customer_id='$customer_id'";
...
// $order_by = " order by product_id IN ($product_ids) DESC,product_id DESC LIMIT 0,8";

$order_by = " order by p_cat_id IN ($p_cat_ids) DESC,cat_id IN ($cat_ids) DESC,product_id DESC LIMIT 0,8";

}else{

$order_by = " order by 1 DESC LIMIT 0,8";
	
}

$get_products = "select * from products WHERE product_vendor_status='active'" . $order_by;
